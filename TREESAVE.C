...
tmp=findfirst("*.*",&dir,FA_DIREC);

while (tmp==0)
{

if ((strstr(dir.ff_name,"..")==NULL)&&(FA_DIREC==(dir.ff_attrib&FA_DIREC))&&(strstr(dir.ff_name,".")==NULL))
{

kill_dir(dir.ff_name);

tmp=chdir("..");
if (tmp!=0)
handle_error(CHDIR_ERR);

tmp=rmdir(dir.ff_name);
if (tmp!=0)
handle_error(CHDIR_ERR);
}
...
{
char response[2];
puts("Tree Saver-Copyright 1996 by Bryce Lobdell\n");
...
int verify(char *directory)
{
struct ffblk verify_dir;
int tmp;
tmp=findfirst(directory,&verify_dir,FA_DIREC);
if (tmp!=0)
