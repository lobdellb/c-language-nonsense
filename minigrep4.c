...
if (Argc != 2)  /*  Error control */
   ...
   while (Getline(Buffer) != EOF)  /* Loops until a EOF  */
      ...
      z{
      zif (Buffer[Count] == *Search)
      zz{
      zzCharMatch = 1;

      zz/*  The inner loop starts assuming that the
      zzstrings are the same (Same =1) and if it finds
      zzand characters that does match, it sets same=0. */
      zzfor (Count2 = 0; Count2 < Len; Count2 ++)
      zzzif (Buffer[Count + Count2] != *(Search + Count2))
      zzzzCharMatch = 0;

      zz}

      zif (CharMatch)  /*  if no characters mismatch, prints whole line */
      zz{
      zzCharMatch = 0;
      zzWordMatch = 1;
      zz}
